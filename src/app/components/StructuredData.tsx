const StructuredData = () => {
  const baseUrl = "https://geminiengineeringworks.in";

  // Organization Schema with comprehensive details
  const organizationSchema = {
    "@context": "https://schema.org",
    "@type": "ManufacturingBusiness",
    "@id": `${baseUrl}/#organization`,
    name: "Gemini Engineering Works",
    alternateName: "Gemini Engg Works",
    url: baseUrl,
    logo: `${baseUrl}/logo.webp`,
    image: `${baseUrl}/logo.webp`,
    description:
      "Buy PSC poles (PSC) and RCC poles in Tamilnadu from Gemini Engineering Works, Pudukkottai. Leading manufacturer since 1999. Supplying to Chennai, Coimbatore, Madurai, Trichy, Salem & all districts. Order RCC poles (9m, 150kg), PSC poles (8m-9m, 200-300kg), base plates. IS 1678, IS 2905, IS 7321 certified. TNEB approved supplier. Wholesale & bulk orders welcome.",
    foundingDate: "1999",
    founder: {
      "@type": "Person",
      name: "AS. Manikandan",
    },
    address: [
      {
        "@type": "PostalAddress",
        addressType: "Site",
        streetAddress:
          "SFNO. 27/6F2 Ganesh Nagar, Sathiyamangalam, Kuluthur TK Vellanur",
        addressLocality: "Pudukkottai",
        postalCode: "622501",
        addressRegion: "Tamil Nadu",
        addressCountry: "IN",
      },
      {
        "@type": "PostalAddress",
        addressType: "Office",
        streetAddress: "1157 housing unit, periya nagar",
        addressLocality: "Pudukkottai",
        postalCode: "622003",
        addressRegion: "Tamil Nadu",
        addressCountry: "IN",
      },
    ],
    contactPoint: [
      {
        "@type": "ContactPoint",
        telephone: "+91-63744-48522",
        contactType: "Customer Service",
        email: "geminienggworks@gmail.com",
        areaServed: "IN",
        availableLanguage: ["en", "ta"],
      },
    ],
    sameAs: [],
    areaServed: [
      {
        "@type": "Country",
        name: "India",
      },
      {
        "@type": "State",
        name: "Tamil Nadu",
      },
      {
        "@type": "City",
        name: "Chennai",
      },
      {
        "@type": "City",
        name: "Coimbatore",
      },
      {
        "@type": "City",
        name: "Madurai",
      },
      {
        "@type": "City",
        name: "Tiruchirappalli",
      },
      {
        "@type": "City",
        name: "Salem",
      },
      {
        "@type": "City",
        name: "Tirunelveli",
      },
      {
        "@type": "City",
        name: "Vellore",
      },
      {
        "@type": "City",
        name: "Tirupur",
      },
      {
        "@type": "City",
        name: "Pudukkottai",
      },
      {
        "@type": "City",
        name: "Thanjavur",
      },
      {
        "@type": "City",
        name: "Sivagangai",
      },
      {
        "@type": "City",
        name: "Dindigul",
      },
      {
        "@type": "City",
        name: "Karur",
      },
      {
        "@type": "City",
        name: "Ramanathapuram",
      },
      {
        "@type": "City",
        name: "Nagapattinam",
      },
      {
        "@type": "City",
        name: "Ariyalur",
      },
      {
        "@type": "City",
        name: "Perambalur",
      },
      {
        "@type": "City",
        name: "Erode",
      },
      {
        "@type": "City",
        name: "Thoothukudi",
      },
      {
        "@type": "City",
        name: "Virudhunagar",
      },
      {
        "@type": "City",
        name: "Cuddalore",
      },
      {
        "@type": "City",
        name: "Villupuram",
      },
      {
        "@type": "City",
        name: "Kanchipuram",
      },
      {
        "@type": "City",
        name: "Tiruvannamalai",
      },
      {
        "@type": "City",
        name: "Theni",
      },
      {
        "@type": "City",
        name: "Kanyakumari",
      },
    ],
    knowsAbout: [
      "Electric Pole Manufacturing",
      "RCC Pole Production",
      "PSC Pole Production",
      "PSC Pole Manufacturing",
      "Pre-stressed Concrete Poles",
      "Reinforced Cement Concrete Poles",
      "TNEB Standards",
      "IS 1678:1998 Standards",
      "IS 2905 Standards",
      "IS 7321 Standards",
      "Power Distribution",
      "Rural Electrification",
      "Street Lighting Poles",
      "HT Line Poles",
      "LT Line Poles",
      "TANGEDCO Approved Pole Manufacturer",
      "Electric Pole Base Plate Supplier",
      "Pole Foundation Plate",
      "PSC Pole vs RCC Pole",
    ],
    slogan: "Planting the Seeds of Light",
  };

  // Local Business Schema
  const localBusinessSchema = {
    "@context": "https://schema.org",
    "@type": "LocalBusiness",
    "@id": `${baseUrl}/#localbusiness`,
    name: "Gemini Engineering Works",
    description:
      "Buy PSC poles (PSC/pre-stressed concrete) and RCC poles (reinforced cement concrete) in Tamil Nadu. TNEB approved manufacturer in Pudukkottai since 1999. Supplying Chennai, Coimbatore, Madurai, Trichy, Salem & all Tamil Nadu districts. Street lighting poles, HT/LT line poles, rural electrification poles. IS 1678, IS 2905 certified.",
    image: `${baseUrl}/logo.webp`,
    url: baseUrl,
    telephone: "+91-63744-48522",
    email: "geminienggworks@gmail.com",
    address: {
      "@type": "PostalAddress",
      streetAddress:
        "SFNO. 27/6F2 Ganesh Nagar, Sathiyamangalam, Kuluthur TK Vellanur",
      addressLocality: "Pudukkottai",
      postalCode: "622501",
      addressRegion: "Tamil Nadu",
      addressCountry: "IN",
    },
    geo: {
      "@type": "GeoCoordinates",
      latitude: "10.4763535",
      longitude: "78.7853814",
    },
    openingHoursSpecification: {
      "@type": "OpeningHoursSpecification",
      dayOfWeek: [
        "Monday",
        "Tuesday",
        "Wednesday",
        "Thursday",
        "Friday",
        "Saturday",
      ],
      opens: "08:00",
      closes: "18:00",
    },
    priceRange: "$$",
  };

  // Detailed Product Schemas
  const productSchemas = [
    {
      "@context": "https://schema.org",
      "@type": "Product",
      name: "9 Metre RCC Pole (150kg) - Buy RCC Pole Tamil Nadu | Reinforced Cement Concrete Pole",
      description:
        "Buy 9-metre RCC Pole (Reinforced Cement Concrete Pole) in Tamil Nadu with 150kg working load. Ideal for LT line distribution, street lighting & rural electrification projects. IS 1678:1998 certified. M25 grade concrete. Supplying Chennai, Coimbatore, Madurai, Trichy, Salem. Buy RCC poles from Gemini Engineering Works, Pudukkottai.",
      image: `${baseUrl}/logo.webp`,
      brand: {
        "@type": "Brand",
        name: "Gemini Engineering Works",
      },
      category: "Electric Pole",
      sku: "RCC-9M-150KG",
      additionalProperty: [
        {
          "@type": "PropertyValue",
          name: "Length",
          value: "9 metres",
        },
        {
          "@type": "PropertyValue",
          name: "Working Load",
          value: "150kg",
        },
        {
          "@type": "PropertyValue",
          name: "Ultimate Load",
          value: "300kg",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Grade",
          value: "M25",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Volume",
          value: "0.285m³",
        },
        {
          "@type": "PropertyValue",
          name: "Weight",
          value: "700kg (approx)",
        },
        {
          "@type": "PropertyValue",
          name: "Top Dimensions",
          value: "12.5cm × 12.5cm",
        },
        {
          "@type": "PropertyValue",
          name: "Bottom Dimensions",
          value: "22.5cm × 22.5cm",
        },
        {
          "@type": "PropertyValue",
          name: "Planting Depth",
          value: "1.70m",
        },
      ],
    },
    {
      "@context": "https://schema.org",
      "@type": "Product",
      name: "9 Metre PSC Pole (300kg WL) - Buy PSC Pole Tamil Nadu | Pre-stressed Concrete Pole",
      description:
        "Buy 9.0 metre PSC pole (PSC/Pre-stressed Cement Concrete pole) in Tamil Nadu with 300kg working load. Ideal for HT/LT line distribution & power transmission. IS 1678, IS 2905 certified. M40 grade concrete, 16 Nos. of 4mm HTS wires. TNEB approved. Supplying Chennai, Coimbatore, Madurai, Trichy, Salem. Buy PSC poles from Gemini Engineering Works, Pudukkottai.",
      image: `${baseUrl}/logo.webp`,
      brand: {
        "@type": "Brand",
        name: "Gemini Engineering Works",
      },
      category: "Electric Pole",
      sku: "PSC-9M-300KG",
      additionalProperty: [
        {
          "@type": "PropertyValue",
          name: "Length",
          value: "9.00m",
        },
        {
          "@type": "PropertyValue",
          name: "Working Load",
          value: "300kg",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Grade",
          value: "M40",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Volume",
          value: "0.243m³",
        },
        {
          "@type": "PropertyValue",
          name: "Weight",
          value: "607kg (approx)",
        },
        {
          "@type": "PropertyValue",
          name: "Top Dimensions",
          value: "18.5cm × 10.0cm",
        },
        {
          "@type": "PropertyValue",
          name: "Bottom Dimensions",
          value: "35.5cm × 10.0cm",
        },
        {
          "@type": "PropertyValue",
          name: "Reinforcement",
          value: "16 Nos. of 4mm HTS wires (all tensioned)",
        },
      ],
    },
    {
      "@context": "https://schema.org",
      "@type": "Product",
      name: "8m PSC Pole (200kg WL) - Buy PSC Pole Tamilnadu | Pre-stressed Concrete Pole",
      description:
        "Buy 8.00m PSC pole (PSC/Pre-stressed Cement Concrete pole) in Tamilnadu with 200kg working load. Ideal for street lighting, LT line distribution & rural electrification. IS 1678, IS 2905 certified. M42 grade concrete, 14 Nos. of 4mm HTS wires. Supplying Chennai, Coimbatore, Madurai, Trichy, Salem. Buy PSC poles from Gemini Engineering Works, Pudukkottai.",
      image: `${baseUrl}/logo.webp`,
      brand: {
        "@type": "Brand",
        name: "Gemini Engineering Works",
      },
      category: "Electric Pole",
      sku: "PSC-8M-200KG",
      additionalProperty: [
        {
          "@type": "PropertyValue",
          name: "Length",
          value: "8.00m",
        },
        {
          "@type": "PropertyValue",
          name: "Working Load",
          value: "200kg",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Grade",
          value: "M42",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Volume",
          value: "0.156m³",
        },
        {
          "@type": "PropertyValue",
          name: "Weight",
          value: "378kg (approx)",
        },
        {
          "@type": "PropertyValue",
          name: "Top Dimensions",
          value: "14.5cm × 9cm",
        },
        {
          "@type": "PropertyValue",
          name: "Bottom Dimensions",
          value: "29.0cm × 9cm",
        },
        {
          "@type": "PropertyValue",
          name: "Thickness",
          value: "9.0cm",
        },
        {
          "@type": "PropertyValue",
          name: "Reinforcement",
          value: "14 Nos. of 4mm HTS wires (12 tensioned, 2 untensioned)",
        },
      ],
    },
    {
      "@context": "https://schema.org",
      "@type": "Product",
      name: "RCC Base Plate 450mm × 450mm × 75mm - Buy Pole Foundation Plate Tamil Nadu",
      description:
        "Buy base plate (pole foundation plate/anchor plate) in Tamil Nadu for electric pole foundation support. Buy RCC base plates in Chennai, Coimbatore, Madurai, Trichy, Salem & all Tamilnadu districts from Gemini Engineering Works. High-quality concrete base plates for PSC and RCC pole installation. M15 grade concrete, HTS wire reinforcement.",
      image: `${baseUrl}/logo.webp`,
      brand: {
        "@type": "Brand",
        name: "Gemini Engineering Works",
      },
      category: "Base Plate",
      sku: "RCC-BASE-450X450X75",
      additionalProperty: [
        {
          "@type": "PropertyValue",
          name: "Dimensions",
          value: "450mm × 450mm × 75mm",
        },
        {
          "@type": "PropertyValue",
          name: "Weight",
          value: "37.40kg (approx)",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Grade",
          value: "M15",
        },
        {
          "@type": "PropertyValue",
          name: "Concrete Volume",
          value: "0.0152m³",
        },
        {
          "@type": "PropertyValue",
          name: "Reinforcement",
          value: "4mm HTS wire @ 50mm c/c",
        },
        {
          "@type": "PropertyValue",
          name: "Clear Cover",
          value: "25mm",
        },
      ],
    },
  ];

  // Service Schema
  const serviceSchema = {
    "@context": "https://schema.org",
    "@type": "Service",
    serviceType: "PSC & RCC Pole Manufacturing Tamil Nadu",
    provider: {
      "@type": "ManufacturingBusiness",
      name: "Gemini Engineering Works",
    },
    areaServed: [
      {
        "@type": "State",
        name: "Tamil Nadu",
      },
      {
        "@type": "Country",
        name: "India",
      },
      {
        "@type": "City",
        name: "Chennai",
      },
      {
        "@type": "City",
        name: "Coimbatore",
      },
      {
        "@type": "City",
        name: "Madurai",
      },
      {
        "@type": "City",
        name: "Tiruchirappalli",
      },
      {
        "@type": "City",
        name: "Salem",
      },
      {
        "@type": "City",
        name: "Tirunelveli",
      },
      {
        "@type": "City",
        name: "Pudukkottai",
      },
      {
        "@type": "City",
        name: "Thanjavur",
      },
      {
        "@type": "City",
        name: "Sivagangai",
      },
      {
        "@type": "City",
        name: "Dindigul",
      },
      {
        "@type": "City",
        name: "Karur",
      },
      {
        "@type": "City",
        name: "Ramanathapuram",
      },
      {
        "@type": "City",
        name: "Nagapattinam",
      },
      {
        "@type": "City",
        name: "Ariyalur",
      },
      {
        "@type": "City",
        name: "Erode",
      },
      {
        "@type": "City",
        name: "Thoothukudi",
      },
      {
        "@type": "City",
        name: "Virudhunagar",
      },
      {
        "@type": "City",
        name: "Kanchipuram",
      },
      {
        "@type": "City",
        name: "Tiruvannamalai",
      },
      {
        "@type": "City",
        name: "Theni",
      },
      {
        "@type": "City",
        name: "Kanyakumari",
      },
    ],
    description:
      "Buy PSC poles (PSC/pre-stressed concrete) and RCC poles (reinforced cement concrete) in Tamil Nadu. Manufacturing high-quality electric poles for power distribution, street lighting, HT/LT lines, and rural electrification. Supplying Chennai, Coimbatore, Madurai, Trichy, Salem & all Tamil Nadu districts. IS 1678, IS 2905, IS 7321 certified. TNEB & TANGEDCO approved.",
    offers: {
      "@type": "Offer",
      description:
        "Buy PSC poles and RCC poles in Tamil Nadu. Bulk supply for government and private projects.",
    },
  };

  // FAQ Schema
  const faqSchema = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    mainEntity: [
      {
        "@type": "Question",
        name: "What types of electric poles do you manufacture?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "We manufacture RCC poles (Reinforced Cement Concrete poles) and PSC poles (PSC/Pre-stressed Cement Concrete poles). Our product range includes 9m RCC poles with 150kg working load for street lighting and LT distribution, 8m PSC poles with 200kg working load, 9m PSC poles with 300kg working load for HT/LT distribution, and RCC base plates (pole foundation plates) 450mm × 450mm × 75mm. All poles suitable for rural electrification and power distribution projects.",
        },
      },
      {
        "@type": "Question",
        name: "Are your poles compliant with industry standards?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, all our PSC (PSC) and RCC poles are manufactured as per IS 1678:1998, IS 2905, and IS 7321 Indian Standards. They undergo rigorous load testing, bending tests, and crack resistance tests before dispatch. We also comply with TNEB (Tamil Nadu Electricity Board) specifications. Our pre-stressed concrete poles and reinforced cement concrete poles meet all BIS certification requirements.",
        },
      },
      {
        "@type": "Question",
        name: "What is the lifespan of your electric poles?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Our RCC and PSC poles are corrosion-free and require minimal maintenance. With a lifespan of 30-40 years, they offer excellent value for power distribution and electrification projects.",
        },
      },
      {
        "@type": "Question",
        name: "Do you supply to TNEB yards?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, we are a proud supplier to TNEB (Tamil Nadu Electricity Board) yards. We serve power distribution companies and contractors across regions, supporting both government and private infrastructure projects.",
        },
      },
      {
        "@type": "Question",
        name: "What is your manufacturing capacity?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "With over 25 years of experience and state-of-the-art manufacturing facilities, we have manufactured over 3 lakh poles. We are capable of meeting large-scale departmental tenders on tight deadlines.",
        },
      },
      {
        "@type": "Question",
        name: "Where is your manufacturing facility located?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Our manufacturing facility is located at SFNO. 27/6F2 Ganesh Nagar, Sathiyamangalam, Kuluthur TK Vellanur, Pudukkottai - 622501, Tamil Nadu. We also have an office at 1157 housing unit, periya nagar, Pudukkottai - 622003.",
        },
      },
      {
        "@type": "Question",
        name: "How can I buy RCC or PSC poles?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "You can buy RCC and PSC poles by contacting us at +91 63744 48522 or emailing geminienggworks@gmail.com. We accept bulk orders and provide competitive pricing. Visit our contact page to get a quote or place an order.",
        },
      },
      {
        "@type": "Question",
        name: "Can I buy electric poles online?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, you can buy electric poles online by contacting us through our website. Call +91 63744 48522 or email geminienggworks@gmail.com to place your order. We provide quotes and accept bulk orders for RCC and PSC poles.",
        },
      },
      {
        "@type": "Question",
        name: "What is the price of RCC and PSC poles?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Pole prices vary based on specifications, quantity, and delivery location. Contact us at +91 63744 48522 or email geminienggworks@gmail.com for competitive pricing quotes. We offer special rates for bulk orders.",
        },
      },
      {
        "@type": "Question",
        name: "Do you deliver poles after purchase?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, we provide delivery services for purchased poles. We can arrange transportation to your location. Contact us at +91 63744 48522 to discuss delivery options and get a quote including delivery charges.",
        },
      },
      {
        "@type": "Question",
        name: "Where can I buy PSC poles in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "You can buy PSC poles in Tamil Nadu from Gemini Engineering Works, Pudukkottai. We are a leading PSC pole manufacturer in Tamilnadu since 1999. Contact us at +91 63744 48522 to buy PSC poles. We offer 8m PSC poles (200kg) and 9m PSC poles (300kg).",
        },
      },
      {
        "@type": "Question",
        name: "Where can I buy RCC poles in Tamilnadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Buy RCC poles in Tamilnadu from Gemini Engineering Works, located in Pudukkottai, Tamil Nadu. We manufacture high-quality 9m RCC poles with 150kg working load. Call +91 63744 48522 to place your order for RCC poles.",
        },
      },
      {
        "@type": "Question",
        name: "Which is the best PSC pole manufacturer in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Gemini Engineering Works is a trusted PSC pole manufacturer in Tamil Nadu with over 25 years of experience. Based in Pudukkottai, we are TNEB approved and have manufactured over 3 lakh poles. Buy PSC poles in tamilnadu from us for quality and reliability.",
        },
      },
      {
        "@type": "Question",
        name: "Which is the best RCC pole supplier in Tamilnadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Gemini Engineering Works is among the best RCC pole suppliers in Tamilnadu. Located in Pudukkottai, Tamil Nadu, we manufacture IS 1678:1998 compliant RCC poles. Buy RCC poles in tamil nadu from us for competitive pricing and quality assurance.",
        },
      },
      {
        "@type": "Question",
        name: "Where to buy PSC poles in TamilNadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "You can buy PSC poles in Tamil Nadu from Gemini Engineering Works in Pudukkottai. We are TNEB approved manufacturers since 1999. Call +91 63744 48522 for pricing and availability.",
        },
      },
      {
        "@type": "Question",
        name: "Where to buy RCC poles in TamilNadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Gemini Engineering Works is a leading RCC pole manufacturer in Tamil Nadu, based in Pudukkottai. We supply quality poles across the state. Call +91 63744 48522 to place your order.",
        },
      },
      {
        "@type": "Question",
        name: "What is PSC pole price in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "PSC pole prices in Tamil Nadu vary based on size and specifications. We offer 8m PSC poles (200kg) and 9m PSC poles (300kg). Contact +91 63744 48522 for competitive pricing quotes.",
        },
      },
      {
        "@type": "Question",
        name: "What is RCC pole price in Tamilnadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "RCC pole prices depend on specifications and quantity. We manufacture 9m RCC poles (150kg working load). Contact +91 63744 48522 for current rates and bulk order discounts.",
        },
      },
      {
        "@type": "Question",
        name: "How to purchase PSC pole in Tamilnadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Contact Gemini Engineering Works at +91 63744 48522 or email geminienggworks@gmail.com. We are TNEB approved manufacturers in Pudukkottai. You can purchase directly from our facility—bulk orders are welcome.",
        },
      },
      {
        "@type": "Question",
        name: "How to purchase RCC pole in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Call +91 63744 48522 to purchase RCC poles directly from Gemini Engineering Works. We offer 9m RCC poles (150kg working load) with delivery across Tamil Nadu.",
        },
      },
      {
        "@type": "Question",
        name: "Can I purchase PSC poles directly from manufacturer in TamilNadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, you can purchase PSC poles directly from Gemini Engineering Works. As manufacturers since 1999, we offer competitive rates without middlemen. Visit our Pudukkottai facility or call +91 63744 48522.",
        },
      },
      {
        "@type": "Question",
        name: "What is the minimum quantity to purchase electric poles?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "You can purchase electric poles in any quantity from Gemini Engineering Works. Whether you need to purchase a single PSC pole or RCC pole, or bulk purchase for large projects, we accommodate all orders. Contact +91 63744 48522 to discuss your purchase requirements in Tamil Nadu.",
        },
      },
      {
        "@type": "Question",
        name: "Where to purchase PSC pole in Pudukkottai district?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Purchase PSC pole in Pudukkottai district from Gemini Engineering Works. Our manufacturing facility is at SFNO. 27/6F2 Ganesh Nagar, Sathiyamangalam, Kuluthur TK Vellanur, Pudukkottai. Call +91 63744 48522 to purchase PSC poles directly from our Pudukkottai factory.",
        },
      },
      {
        "@type": "Question",
        name: "Is it easy to purchase poles for TNEB projects?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Yes, purchasing poles for TNEB projects is easy with Gemini Engineering Works. We are TNEB approved suppliers. Purchase PSC poles and RCC poles that meet all TNEB specifications. Call +91 63744 48522 to purchase poles for your TNEB contracted projects in Tamilnadu.",
        },
      },
      {
        "@type": "Question",
        name: "Where can I buy base plate in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Buy base plate in Tamil Nadu from Gemini Engineering Works, Pudukkottai. We manufacture high-quality RCC base plates (450mm × 450mm × 75mm) with M15 grade concrete. Buy base plate tamilnadu for electric pole foundation. Call +91 63744 48522 to buy base plates.",
        },
      },
      {
        "@type": "Question",
        name: "Where to buy RCC base plate in Tamilnadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Buy RCC base plate in Tamilnadu from Gemini Engineering Works. We are leading base plate manufacturers in Tamil Nadu. Buy base plate for PSC and RCC pole installation. Our base plates come with 4mm HTS wire reinforcement @ 50mm c/c. Call +91 63744 48522 to buy base plates.",
        },
      },
      {
        "@type": "Question",
        name: "What is the price of base plate in Tamil Nadu?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Base plate price in Tamil Nadu varies based on quantity. Buy base plate from Gemini Engineering Works at competitive prices. Our RCC base plates (450mm × 450mm × 75mm) weigh 37.40kg. Contact +91 63744 48522 for base plate price tamilnadu and bulk order discounts.",
        },
      },
    ],
  };

  // WebPage Schema for Home
  const webPageSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    "@id": `${baseUrl}/#webpage`,
    url: baseUrl,
    name: "Buy RCC Pole, PSC Pole, Electric Poles & Base Plate in Tamil Nadu (TN) | Gemini Engineering Works",
    description:
      "Buy RCC pole, PSC pole, electric poles & base plate in Tamil Nadu (TN). Buy poles & baseplates in Tamilnadu from Gemini Engineering Works. TNEB approved manufacturer. Call +91 63744 48522.",
    inLanguage: "en-IN",
    isPartOf: {
      "@type": "WebSite",
      name: "Gemini Engineering Works - Buy RCC Pole PSC Pole Electric Poles Base Plate Tamil Nadu TN",
      url: baseUrl,
    },
    about: {
      "@type": "Thing",
      name: "Buy RCC Pole PSC Pole Electric Poles and Base Plate in Tamil Nadu TN",
    },
    primaryImageOfPage: {
      "@type": "ImageObject",
      url: `${baseUrl}/logo.webp`,
    },
  };

  // Breadcrumb Schema
  const breadcrumbSchema = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      {
        "@type": "ListItem",
        position: 1,
        name: "Home",
        item: baseUrl,
      },
    ],
  };

  // ItemList Schema for Products
  const itemListSchema = {
    "@context": "https://schema.org",
    "@type": "ItemList",
    name: "Buy PSC & RCC Poles in Tamil Nadu",
    description:
      "Buy PSC poles and RCC poles in Tamil Nadu. Complete range of electric poles manufactured by Gemini Engineering Works, Pudukkottai, Tamilnadu.",
    itemListElement: [
      {
        "@type": "ListItem",
        position: 1,
        name: "9 Metre RCC Pole (150kg)",
        url: `${baseUrl}/#products`,
      },
      {
        "@type": "ListItem",
        position: 2,
        name: "9 metre PSC Pole (300kg WL)",
        url: `${baseUrl}/#products`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: "8 metre PSC Pole (200kg WL)",
        url: `${baseUrl}/#products`,
      },
      {
        "@type": "ListItem",
        position: 4,
        name: "RCC Base Plate 450mm × 450mm × 75mm - Buy Base Plate Tamil Nadu",
        url: `${baseUrl}/#products`,
      },
    ],
  };

  // HowTo Schema for Purchase Process - targets "purchase psc pole tamilnadu"
  const howToPurchasePSCSchema = {
    "@context": "https://schema.org",
    "@type": "HowTo",
    name: "How to Purchase PSC Pole in Tamilnadu",
    description:
      "Step-by-step guide to purchase PSC poles in Tamil Nadu from Gemini Engineering Works, the leading PSC pole manufacturer in Pudukkottai.",
    totalTime: "PT30M",
    estimatedCost: {
      "@type": "MonetaryAmount",
      currency: "INR",
      value: "Contact for pricing",
    },
    supply: [
      {
        "@type": "HowToSupply",
        name: "Project specifications",
      },
      {
        "@type": "HowToSupply",
        name: "Quantity requirements",
      },
    ],
    tool: [
      {
        "@type": "HowToTool",
        name: "Phone or Email for contact",
      },
    ],
    step: [
      {
        "@type": "HowToStep",
        name: "Contact Gemini Engineering Works",
        text: "Call +91 63744 48522 or email geminienggworks@gmail.com to purchase PSC poles in Tamilnadu",
        url: `${baseUrl}/#contact`,
        position: 1,
      },
      {
        "@type": "HowToStep",
        name: "Specify Your Requirements",
        text: "Tell us the PSC pole specifications: 8m (200kg) or 9m (300kg) and quantity needed for your purchase",
        position: 2,
      },
      {
        "@type": "HowToStep",
        name: "Get a Quote",
        text: "Receive competitive pricing for your PSC pole purchase in Tamil Nadu",
        position: 3,
      },
      {
        "@type": "HowToStep",
        name: "Confirm Purchase Order",
        text: "Confirm your purchase order for PSC poles with delivery details",
        position: 4,
      },
      {
        "@type": "HowToStep",
        name: "Receive Delivery",
        text: "Get your purchased PSC poles delivered to your location in Tamilnadu",
        position: 5,
      },
    ],
  };

  // HowTo Schema for Purchase RCC Poles
  const howToPurchaseRCCSchema = {
    "@context": "https://schema.org",
    "@type": "HowTo",
    name: "How to Purchase RCC Pole in Tamil Nadu",
    description:
      "Complete guide to purchase RCC poles in Tamil Nadu from Gemini Engineering Works, trusted RCC pole manufacturer since 1999.",
    totalTime: "PT30M",
    step: [
      {
        "@type": "HowToStep",
        name: "Contact for RCC Pole Purchase",
        text: "Call +91 63744 48522 to purchase RCC poles in Tamil Nadu. Email: geminienggworks@gmail.com",
        url: `${baseUrl}/#contact`,
        position: 1,
      },
      {
        "@type": "HowToStep",
        name: "Specify RCC Pole Requirements",
        text: "Provide specifications for 9m RCC poles (150kg working load) and quantity for purchase",
        position: 2,
      },
      {
        "@type": "HowToStep",
        name: "Receive Purchase Quote",
        text: "Get competitive pricing quote for RCC pole purchase in Tamilnadu",
        position: 3,
      },
      {
        "@type": "HowToStep",
        name: "Complete Purchase",
        text: "Finalize your RCC pole purchase order with Gemini Engineering Works",
        position: 4,
      },
    ],
  };

  // Speakable Schema for Voice Search
  const speakableSchema = {
    "@context": "https://schema.org",
    "@type": "WebPage",
    speakable: {
      "@type": "SpeakableSpecification",
      cssSelector: ["h1", "h2", ".hero-text"],
    },
    name: "Purchase PSC Pole Tamilnadu - Gemini Engineering Works",
    description:
      "Purchase PSC poles and RCC poles in Tamilnadu from Gemini Engineering Works. Call +91 63744 48522 to purchase electric poles in Tamil Nadu.",
  };

  // Store Schema for E-commerce visibility
  const storeSchema = {
    "@context": "https://schema.org",
    "@type": "Store",
    name: "Gemini Engineering Works - Buy PSC & RCC Poles Tamil Nadu",
    description:
      "Buy PSC poles and RCC poles in Tamil Nadu. Leading electric pole manufacturer in Pudukkottai, Tamilnadu since 1999. TNEB approved supplier.",
    image: `${baseUrl}/logo.webp`,
    "@id": `${baseUrl}/#store`,
    url: baseUrl,
    telephone: "+91-63744-48522",
    email: "geminienggworks@gmail.com",
    address: {
      "@type": "PostalAddress",
      streetAddress:
        "SFNO. 27/6F2 Ganesh Nagar, Sathiyamangalam, Kuluthur TK Vellanur",
      addressLocality: "Pudukkottai",
      postalCode: "622501",
      addressRegion: "Tamil Nadu",
      addressCountry: "IN",
    },
    geo: {
      "@type": "GeoCoordinates",
      latitude: "10.4763535",
      longitude: "78.7853814",
    },
    openingHoursSpecification: {
      "@type": "OpeningHoursSpecification",
      dayOfWeek: [
        "Monday",
        "Tuesday",
        "Wednesday",
        "Thursday",
        "Friday",
        "Saturday",
      ],
      opens: "08:00",
      closes: "18:00",
    },
    priceRange: "$$",
    paymentAccepted: "Cash, Bank Transfer, Cheque",
    currenciesAccepted: "INR",
  };

  return (
    <>
      {/* Organization Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(organizationSchema) }}
      />
      {/* Local Business Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(localBusinessSchema),
        }}
      />
      {/* Product Schemas */}
      {productSchemas.map((schema, index) => (
        <script
          key={index}
          type="application/ld+json"
          dangerouslySetInnerHTML={{ __html: JSON.stringify(schema) }}
        />
      ))}
      {/* Service Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(serviceSchema) }}
      />
      {/* FAQ Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
      />
      {/* WebPage Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(webPageSchema) }}
      />
      {/* Breadcrumb Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }}
      />
      {/* ItemList Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(itemListSchema) }}
      />
      {/* Store Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(storeSchema) }}
      />
      {/* HowTo Schema - Purchase PSC Pole */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(howToPurchasePSCSchema),
        }}
      />
      {/* HowTo Schema - Purchase RCC Pole */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{
          __html: JSON.stringify(howToPurchaseRCCSchema),
        }}
      />
      {/* Speakable Schema */}
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(speakableSchema) }}
      />
    </>
  );
};

export default StructuredData;
